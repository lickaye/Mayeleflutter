...
class pubthree extends StatefulWidget {
  const pubthree({Key? key}) : super(key: key);

  @override
  _pubthreeState createState() => _pubthreeState();
}

class _pubthreeState extends State<pubthree> {
  List contenu = [];
  ...
  fetchContenu() async {
    ...
    if (response.statusCode == 200) {
      var items = json.decode(response.body)['Publicite']['pubThree'];
      setState(() {
        contenu = items;
        isLoadibg = true;
      });
